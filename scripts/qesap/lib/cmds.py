...
def cmd_destroy(configure_data, base_project, dryrun=False, verbose=False):
    ...
    if res != 0:
        return res
    return cmd_terraform(configure_data, base_project, dryrun, workspace='default', destroy=False)
...
def ansible_command_sequence(configure_data_ansible, base_project, sequence, verbose, inventory, profile, junit):
    ...
    # This is a strategy to share the ssh host fingerprint,
    # it is a way to run the first connection without the need to interactivly
    # intervent to accept and recognize the remote host.
    # It is implemented using https://docs.ansible.com/ansible/latest/command_guide/intro_adhoc.html
    # So used binary is ansible and not ansible-playbook
    # 'all' is about to run the same command an all hosts in the inventory (that come from ansible_common)
    # `-a true` is about to run the simplest command possible remotely,
    # as we not care what we run there, but to extablish the first connection
